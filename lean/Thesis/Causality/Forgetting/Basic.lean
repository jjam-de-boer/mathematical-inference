...
equation. Exogenous deletion removes a latent root that is incident to no
observed mechanism and pushes the prior and current belief forward along the
coordinate projection.

This module contains the executable deletion constructions and their semantic
preservation results. The model-extension argument needed for two-way observed
identifiability is isolated in `Thesis.Causality.Forgetting.Identification` so
structural-edit clients do not import it merely to construct a deletion.
-/
